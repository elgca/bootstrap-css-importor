...
object CodeBrowser {
  ...
  def listFilesRecursive(path: Path): List[Path] = {
    if (!Files.exists(path)) {
      throw new Exception(
        s"listFilesRecursive: ${path.toAbsolutePath} does not exist"
      )
    } else {
      walk(path)
    }
  ...
  def findCodeSnippets(
    rootPath: Path,
    fileFilter: Path => Boolean
  ): Map[String, List[SbtCodeSnippet]] = {
